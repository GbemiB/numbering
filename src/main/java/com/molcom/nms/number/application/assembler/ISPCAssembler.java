...
@Slf4j
@Component
public class ISPCAssembler {
    @Autowired
    private ApprovalRepository approvalRepository;

    public ISPCNumberObject ispcNumberObjectBuilder(ISPCNumberModel model) {

        ISPCNumberObject response = new ISPCNumberObject();
        try {
            ...
            response.setExistingIspcNumber(model.getExistingIspcNumber());
            response.setExistingIspcNumberId(model.getExistingIspcNumberId());
            response.setIntendingIspcNumber(model.getIntendingIspcNumber());
